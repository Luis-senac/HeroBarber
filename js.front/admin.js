document.addEventListener('DOMContentLoaded', async function() {
    ...
    try {
        ...
        // **ATENÇÃO:** Como não temos a API real, vou simular alguns dados para teste:
        agendamentos = [
            { nome: 'Cliente A', email: 'a@exemplo.com', profissional: 'Prof 1', data: '28/10/2025', horario: '10:00' },
            { nome: 'Cliente B', email: 'b@exemplo.com', profissional: 'Prof 2', data: '28/10/2025', horario: '14:30' },
            { nome: 'Cliente C', email: 'c@exemplo.com', profissional: 'Prof 1', data: '05/11/2025', horario: '09:00' }
        ];
    ...
    agendamentos = agendamentos.map(a => ({
        ...
        data: a.data || '-', // Deve estar no formato DD/MM/AAAA
    ...
    function gerarCalendario(data) {
        ...
        mesAnoTitulo.textContent = `${nomeMes.charAt(0).toUpperCase() + nomeMes.slice(1)} ${ano}`;
    ...
    function abrirModal(dataCompleta, celula) {
        ...
        const agendamentosDoDia = agendamentos.filter(a => a.data === dataCompleta);
        agendamentosDoDiaDiv.innerHTML = ''; 

        if (agendamentosDoDia.length === 0) {
            agendamentosDoDiaDiv.innerHTML = '<p>Nenhum agendamento encontrado para esta data.</p>';
        } else {
        ...
        const classesDeStatus = ['dia-status-feito', 'dia-status-pendente', 'dia-status-negado'];
    ...
    function salvarStatus() {
        ...
        const classesDeStatusRemover = ['dia-status-feito', 'dia-status-pendente', 'dia-status-negado', 'dia-selecionado'];
